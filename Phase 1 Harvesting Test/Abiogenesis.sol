...
import "./NanitesTest.sol";
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/access/Ownable.sol";

contract Abiogenesis is Ownable {
    // admin address
    address private admin;
    // balance of tokens held in escrow
    uint256 public contractBalance;
    // this is the erc20 NanitesToken contract address
    address constant tokenAddress = 0xbB4D9d9CF954AaC08B28D7e1bBF65BD1b16d3683; // <-- INSERT DEPLYED ERC20 TOKEN CONTRACT HERE
    uint256 public maxSupply =  1000000000000000000000; // <-- temporar1000ily set manually for flexibility while in pre-alpha development
    uint256 public unit =  1000000000000000000000; // <-- temporaily set manually for flexibility while in pre-alpha development
    ...
    function createGame(
        ...
        NanitesToken token = NanitesToken(tokenAddress);
        //unit = token.unit();
    ...
    function playerWon(uint256 _gameId, address _player)
        ...
        returns (bool)
    {
        Nanites token = NanitesToken(tokenAddress);
